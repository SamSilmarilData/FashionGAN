...
def subprocess_fn(rank, args, temp_dir):
    ...
    num_nodes = 2
    local_rank = rank
    global_rank = int(local_rank + int(os.environ['SLURM_PROCID']) * (args.num_gpus // num_nodes))
    local_gpus = args.num_gpus // num_nodes
    ...
    # Init torch_utils.
    sync_device = torch.device('cuda', local_rank) if args.num_gpus > 1 else None
    training_stats.init_multiprocessing(rank=global_rank, sync_device=sync_device)
    if local_rank != 0:
        custom_ops.verbosity = 'none'

    # Execute training loop.
    training_loop.training_loop(rank=rank, local_rank=local_rank, global_rank=global_rank, local_gpus=local_gpus, global_gpus=global_gpus, **args)
...
def main(ctx, outdir, dry_run, **config_kwargs):
    ...
    args.run_dir = os.path.join(outdir, f'{cur_run_id}-{run_desc}')
